...
while 1:
    ...
    def renderMap():
        ...
        for enemies in enemyArr:
            ...
            enemies['hp'] = float("{0:.2f}".format(enemies['hp']))
            enemies['att'] = float("{0:.2f}".format(enemies['att']))
            enemies['def'] = float("{0:.2f}".format(enemies['def']))
        #Clear screen
        screen.fill(black) 
        ...
        if (hero_stats['hp'] <= 0):
            console_log.append({'log' : 'You dieded, press r to restart', 'id' : 5})
            global dead
            dead = True
        # create rectangles
        ...
        # console log
        # id 0 = passive, 1 = combat, 2 = xp, 5 = important
        count = 0
    ...
    while rend:
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT: 
                ...
                if event.key == pygame.K_w and dead == False:
                    # print('Pressed w')
                    moveScreen('w')
                elif event.key == pygame.K_s and dead == False:
                    # print('Pressed s')
                    moveScreen('s') 
                elif event.key == pygame.K_a and dead == False:
                    # print('Pressed a')
                    moveScreen('a') 
                elif event.key == pygame.K_d and dead == False:
                    # print('Pressed d')
                    moveScreen('d')
                elif event.key == pygame.K_p and dead == False:
                    # attack
                    moveScreen('p') 
