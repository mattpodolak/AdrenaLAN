...
def loadEnemies(room_arr, num_enemies, enemy_type, player, enemy_arr):
    ...
    # different types
    if enemy_type == 'Starboi':
        base_hp = 10
        base_att = 6
        base_def = 2
    elif enemy_type == 'Demondog':
        base_hp = 4
        base_att = 2
        base_def = 1
    elif enemy_type == 'Hells Janitor':
        base_hp = 20
        base_att = 3
        base_def = 6
    ...
    while (start_counter != num_enemies):
        ...
        if chance <= 30:
            ...
            stat_att = float("{0:.2f}".format(createStats(base_att, 1)))
            stat_def = float("{0:.2f}".format(createStats(base_def, 1)))
            stat_elem = None
            crit_dmg = 1.5
            crit_chc = 25
            default_size = 1
            ...
            enemy_arr.append({'name' : enemy_type, 'x_loc' : x_pos, 'y_loc' : y_pos, 'hp' : stat_hp, 'att' : stat_att, 'def' : stat_def, 'elem' : stat_elem, 'crit_dmg' : crit_dmg, 'crit_chc' : crit_chc, 'mutations' : stat_mut, 'size' : default_size, 'xp' : default_xp})
